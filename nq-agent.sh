...
then
	timeout -s SIGKILL 30 wget -q -o /dev/null -O /etc/nodequery/nq-agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://api01-a224327780.cloud.okteto.net/agent/save"
else
	wget -q -o /dev/null -O /etc/nodequery/nq-agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://api01-a224327780.cloud.okteto.net/agent/save"
	wget_pid=$! 
	wget_counter=0
	wget_timeout=30
